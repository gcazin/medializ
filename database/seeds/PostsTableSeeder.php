...
class PostsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                'name'                  => 'posts',
                'display_name_singular' => __('voyager::seeders.data_types.admin.singular'),
                'display_name_plural'   => __('voyager::seeders.data_types.admin.plural'),
                'icon'                  => 'voyager-news',
        ...
        //Content
        $post = $this->findPost('lorem-ipsum-admin');
        if (!$post->exists) {
            $post->fill([
                ...
                'body'             => '<p>This is the body of the lorem ipsum admin</p>',
                'image'            => 'posts/post1.jpg',
                'slug'             => 'lorem-ipsum-admin',
        ...
        $post = $this->findPost('my-sample-admin');
        if (!$post->exists) {
            $post->fill([
                ...
                'body'      => '<p>This is the body for the sample admin, which includes the body.</p>
                <h2>We can use all kinds of format!</h2>
                <p>And include a bunch of other stuff.</p>',
                'image'            => 'posts/post2.jpg',
                'slug'             => 'my-sample-admin',
                'meta_description' => 'Meta Description for sample admin',
                'meta_keywords'    => 'keyword1, keyword2, keyword3',
                'status'           => 'PUBLISHED',
                'featured'         => 0,
            ])->save();
        ...
        $post = $this->findPost('latest-admin');
        if (!$post->exists) {
            $post->fill([
                'title'            => 'Latest Post',
                'author_id'        => 0,
                'seo_title'        => null,
                'excerpt'          => 'This is the excerpt for the latest admin',
                'body'             => '<p>This is the body for the latest admin</p>',
