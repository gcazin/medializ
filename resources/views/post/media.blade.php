<div class="w-full lg:max-w-full lg:flex mb-4 lg:mr-4 dark:shadow-2xl shadow relative">
    <a href="{{ route('detail', [$post->id, $post->slug]) }}" class="absolute w-full h-full"></a>
    ...
    <div class="lg:w-9/12 dark:bg-gray-800 bg-white rounded-r lg:justify-between leading-normal">
        ...
        <div class="p-4 flex items-center bg-gray-200 dark:bg-gray-900">
            ...
            @if(\Illuminate\Support\Facades\Auth::check())
                @if(\App\User::isAdmin())
                    <a href="{{ route('post.update', $post->id) }}" class="btn btn-light btn-sm">Modifier</a>
                    <a href="{{ route('post.delete', $post->id) }}" class="btn btn-red btn-sm">Supprimer le post</a>
                @endif
            @endif
        </div>
    </div>
